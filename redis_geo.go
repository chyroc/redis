...
func (r *Redis) GeoAdd(key string, geo GeoLocation, geos ...GeoLocation) (int, error) {
	args := []string{"GEOADD", key, float64ToString(geo.Longitude), float64ToString(geo.Latitude), geo.Member}
	for _, v := range geos {
		args = append(args, float64ToString(v.Longitude), float64ToString(v.Latitude), v.Member)
	}
...
func (r *Redis) GeoPos(key, member string, members ...string) ([]*GeoLocation, error) {
	return r.run(buildSlice3("GEOPOS", key, member, members)...).fixGeoLocationSlice()
}
...
func (r *Redis) GeoDist(key, member1, member2 string, unit ...string) (float64, error) {
	if len(unit) > 1 {
		return 0, fmt.Errorf("expect 0 or 1 unit")
	}
	if len(unit) > 0 {
		return r.run("GEODIST", key, member1, member2, unit[0]).fixFloat64()
	}
	return r.run("GEODIST", key, member1, member2).fixFloat64()
}
...
func (r *Redis) GeoHash(key, member string, members ...string) ([]string, error) {
	return r.run(buildSlice3("GEOHASH", key, member, members)...).fixStringSlice()
}
